...
def generate_mxlims(dirname: Optional[str] = None) -> None :
    ...
    objects_dir = mxlims_dir / "mxlims" / "impl" / "pydantic" / "objects"
...
def generate_message_classes(mxlims_dir: Path) -> None:
    ...
    :return:
    """
    iput_dir = mxlims_dir / "mxlims" / "pydantic" / "messages"
    output_dir = mxlims_dir / "mxlims" / "impl" / "pydantic" / "messages"
    for fp0 in iput_dir.iterdir():
        if fp0.is_file():
            classname = fp0.stem
            classbody = f'''# generated by mxlims/impl/generate_code.py
#  filename {classname}.py

from mxlims.pydantic.messages.{classname} import {classname} as {classname}_orig
from mxlims.impl.pydantic.MxlimsBase import BaseMessage

class {classname}({classname}_orig, BaseMessage):
    """MxlimsMessage class, combining pydantic class with Implementation class"""
'''
            fpath = output_dir / fp0.name
            with fpath.open("w", encoding="utf-8") as fp1:
                fp1.write(classbody)
